...
минуты у платформы стоит следующий поезд и т. д.
...
Вторая строка содержит число b — интервал между поездами на втором пути. Третья строка содержит
число n — количество поездов на первом пути, которые увидела Таня. Четвёртая строка содержит
число m — количество поездов на втором пути, которые увидела Таня. Все числа — целые, от 1 до 1000.
...
min_a = (interval_a + 1) * count_a - interval_a
max_a = (interval_a + 1) * count_a + interval_a
min_b = (interval_b + 1) * count_b - interval_b
max_b = (interval_b + 1) * count_b + interval_b

result_min = max(min_a, min_b)
result_max = min(max_a, max_b)
